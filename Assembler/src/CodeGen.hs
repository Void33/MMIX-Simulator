...
genCodeForLine :: (M.Map String RegisterAddress) -> (M.Map Char Int) -> Line -> Maybe(Int, Int, [Char])
...
genCodeForLine symbols registers ln@(PlainOpCodeLine opcode (ListIdElement _ (Register r)) address) =
    case (mapSymbolToAddress symbols registers ln) of
        Just(base, offset) -> Just(address, 4, (chr opcode) : (chr r) : base : (chr offset) : [])
        _ -> Nothing
genCodeForLine symbols registers ln@(PlainOpCodeLine opcode (ListElements first second third) address) = Just(address, 4, op : p1 : p2 : p3 : [])
    where op = chr opcode
...
genCodeForLine symbols registers ln@(LabelledOpCodeLine opcode (ListIdElement _ (Register r)) _ address) =
    case (mapSymbolToAddress symbols registers ln) of
        Just(base, offset) -> Just(address, 4, (chr opcode) : (chr r) : base : (chr offset) : [])
        _ -> Nothing
genCodeForLine symbols registers ln@(LabelledOpCodeLine opcode (ListElements first second third) _ address) = Just(address, 4, op : p1 : p2 : p3 : [])
    where op = chr opcode
          p1 = formatElement first
          p2 = formatElement second
          p3 = formatElement third
